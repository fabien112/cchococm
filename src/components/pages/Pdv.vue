    ...
    <div id="app-content">
      ...
      <div class="app-content-area">
        ...
        <div class="container-fluid mt-n22">
          <h2 style="color: #41160a">Point de vente </h2>
          <br />
          <div class="row">
            <!-- <div class="col-4">
            ...
            <div class="col-6">
              ...
              <input placeholder="Saisir un mot cle" type="text" class="form-control" v-model="forms.sNompdv" />
            </div>

            <div class="col-6 pt-7 btn-lg">
              <button style="background-color: #41160a;width: 100%;" @click="show" class="btn btn-primary btn-lg">
                <Icon type="md-arrow-dropright-circle" /> Envoyer
              </button>
            </div>
          </div>
          <br /><br />
                      ...
                      <table class="table text-nowrap mb-0 table-centered table-hover">
                        <thead style="background-color: #41160a;">
                          <tr>
                            ...
                            <th style="color: white">Nom du Pdv </th>
                            <th style="color: white"> Ville </th>
                            <th style="color: white">Region</th>
                            <th style="color: white">Vendeur</th>
                            <th style="color: white"> Gerant</th>
                            <th style="color: white"> Actions </th>
                            <!-- <th style="color: white">Tel gerant</th>
                            <th style="color: white"> Secteur marché</th>
                            <th style="color: white"> Secteur activité </th> -->

                          </tr>
                        </thead>
                        <tbody>
                          <tr v-for="(data, i) in listes.tabListpdvimport" :key="i">
                            <td>{{ data.sNomPDV }}</td>
                            <td>{{ data.sVille }}</td>
                            <td>{{ data.sRegion }}</td>
                            <td>{{ data.sNomuser }}</td>
                            <td>{{ data.sGerantNom }}</td>

                            <td>

                              <router-link to="more">

                                <button @click="more(data)" class="btn btn-primary m-1 btn-sm ">
                                  <Icon type="md-more" style="font-size:25px;" />
                                </button>

                              </router-link>


                              <router-link to="maps2" target="_blank">
                                <button @click="geolocate(data)" title="Geolocalisation du point"
                                  class="btn btn-danger btn-sm m-1">
                                  <Icon type="md-locate" style="font-size:25px;" />
...
export default {
  ...
  data() {
    return {
      forms: {
        ...
        "rLongitude": "",
        "sEgment": "",
        "uiid": "7C8WOWY/mZFNUG66bt90244Ye+Y="
      },
  ...
  methods: {
    ...
    more(item) {
      ...
      localStorage.setItem('users2', JSON.stringify(item));
    ...
    async show() {
      ...
      this.axios
        .post(process.env.VUE_APP_URL_API + "pos/search", this.forms, {
          headers: {
